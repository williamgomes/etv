...
class ReactionRepository extends EntityRepository {
     ...
     */
    public function getReactionsAndResponse($episodeId) {
        $query = $this->getEntityManager()
                ->createQueryBuilder()
                ->select("R1.reactionText, R1.reactionCreatedOn, R1.reactionId, R1.reactionEpisodeId")
                ->addSelect("u.userFirstName,u.userProfilePicture")
                ->addSelect("(SELECT COUNT(R2) FROM EssentialTvEtvBundle:Reactions AS R2 WHERE R2.reactionParentId=R1.reactionId) AS ResponseCount")
                ->from('EssentialTvEtvBundle:Reactions', 'R1')
                ->leftJoin("EssentialTvEtvBundle:Users", "u", "WITH", "u.userId=R1.reactionUserId")
                ->where('R1.reactionEpisodeId = :episodeId')
